...
public class FonctionProgram
{
    ...
    public static void AfficherDes(int[] des, bool[] desGardes)
    {
        ...
        int indexDe = 0;          // compteur pour parcourir les dés

        chaine = "";
        indexDe = 0;
        while (indexDe < 5)
        {
            chaine = chaine + "[" + des[indexDe] + "] ";
            indexDe = indexDe + 1;
        }
        ...
        while (indexDe < 5)
        {
            if (desGardes[indexDe] == true)
            {
                if (chaineGardes == "")
                {
                    chaineGardes = chaineGardes + (indexDe + 1);
                }
                else
                {
                    chaineGardes = chaineGardes + ", " + (indexDe + 1);
    ...
    public static void AfficherFeuilleScore(string nomJoueur, int[] scores, int scoreTotal)
    {
        ...
        int indexCategorie = 0;  // compteur pour parcourir les catégories
        ...
        indexCategorie = 0;
        while (indexCategorie < 6)
        {
            Console.WriteLine(categorieNom[indexCategorie] + " : " + scores[indexCategorie]);
        ...
        while (indexCategorie < 6)
        {
            if (scores[indexCategorie] > 0)
            {
                sousTotal = sousTotal + scores[indexCategorie];
            }
            indexCategorie = indexCategorie + 1;
        }
    ...
    public static void AfficherResultatsFinaux(string[] nomsJoueurs, int nbJoueurs, int[] scoresTotaux)
    {
        ...
        int indexJoueur = 0;   // compteur pour parcourir les joueurs

        indexJoueur = 0;
        while (indexJoueur < nbJoueurs)
        {
            Console.WriteLine(nomsJoueurs[indexJoueur] + " : " + scoresTotaux[indexJoueur] + " points");
        ...
        while (indexJoueur < nbJoueurs)
        {
            if (scoresTotaux[indexJoueur] > maxScore)
            {
                maxScore = scoresTotaux[indexJoueur];
            }
            indexJoueur = indexJoueur + 1;
        }
        ...
        while (indexJoueur < nbJoueurs)
        {
            if (scoresTotaux[indexJoueur] == maxScore)
            {
                Console.WriteLine(nomsJoueurs[indexJoueur] + " a gagné avec " + maxScore + " points");
            }
            indexJoueur = indexJoueur + 1;
        }
    }
    ...
    public static void CalculerBonus(int[] scoresChiffres, out int bonus)
    {
        ...
        int indexChiffre = 0;  // compteur pour parcourir les catégories
        bonus = 0;

        total = 0;
        indexChiffre = 0;
        while (indexChiffre < 6)
        {
            total = total + scoresChiffres[indexChiffre];
            indexChiffre = indexChiffre + 1;
        }
    ...
    public static void CalculerBrelanCarre(int[] des, int type, out int score)
    {
        ...
        int indexDe = 0;             // compteur pour parcourir les dés
        int indexValeur = 0;         // compteur pour parcourir les valeurs 1-6
        score = 0;
        ...
        while (indexDe < 5)
        {
            compteur[des[indexDe]] = compteur[des[indexDe]] + 1;
            indexDe = indexDe + 1;
        }

        trouve = false;
        indexValeur = 1;
        while (indexValeur <= 6 && trouve == false)
        {
            if (type == 3 && compteur[indexValeur] >= 3)
            {
                trouve = true;
            }
            else
            {
                if (type == 4 && compteur[indexValeur] >= 4)
                {
                    trouve = true;
                }
            }
    ...
    {
        int[] compteur = new int[7];  // occurrences de chaque valeur de dé (indices 1-6)
        bool triplePresent = false;   // vrai si une valeur apparaît exactement 3 fois
        bool doublePresent = false;   // vrai si une valeur apparaît exactement 2 fois
        int indexDe = 0;              // compteur pour parcourir les dés
        int indexValeur = 0;          // compteur pour parcourir les valeurs 1-6
        score = 0;
        ...
        while (indexValeur <= 6)
        {
            if (compteur[indexValeur] == 3)
            {
                triplePresent = true;
            }
            else
            {
                if (compteur[indexValeur] == 2)
                {
                    doublePresent = true;
                }
            }
        ...
        if (triplePresent == true && doublePresent == true)
        {
            score = 25;
        }
        else
    ...
    public static void CalculerYahtzee(int[] des, bool premierYahtzee, out int score, out bool yahtzeeDejaFait)
    {
        ...
        yahtzee = false;
        indexValeur = 1;
        while (indexValeur <= 6 && yahtzee == false)
        {
            if (compteur[indexValeur] == 5)
            {
                yahtzee = true;
            }
            indexValeur = indexValeur + 1;
    ...
    public static void ChoisirDesAGarder(ref bool[] desGardes, out int nbAGarder)
    {
        ...
        int indexReset = 0;  // compteur pour réinitialiser desGardes
        int indexGarde = 0;  // compteur pour saisir les dés à garder
        nbAGarder = 0;

        indexReset = 0;
        while (indexReset < 5)
        {
            desGardes[indexReset] = false;
            indexReset = indexReset + 1;
        }
    ...
    public static void GestionJoueurs(int nbJoueurs, out string[] nomsJoueurs, out int[] scoresJoueurs)
    {
        ...
        while (indexJoueur < nbJoueurs)
        {
            Console.WriteLine("Nom du joueur " + (indexJoueur + 1) + " :");
            nomsJoueurs[indexJoueur] = Console.ReadLine() ?? "";
            scoresJoueurs[indexJoueur] = 0;
            indexJoueur = indexJoueur + 1;
        }
    }
    ...
    {
        int indexJoueur = 0;    // compteur pour initialisation
        int indexCategorie = 0; // compteur pour les catégories
        ...
        while (indexJoueur < nbJoueurs)
        {
            scoresJoueurs[indexJoueur] = new int[13];
            indexCategorie = 0;
            while (indexCategorie < 13)
            {
                scoresJoueurs[indexJoueur][indexCategorie] = -1; // -1 = catégorie non encore remplie
                indexCategorie = indexCategorie + 1;
            }
            scoresTotaux[indexJoueur] = 0;
            indexJoueur = indexJoueur + 1;
        }
    }
    ...
    public static void LancerDes(out int[] des, out bool[] desGardes)
    {
        ...
        int indexDe = 0;            // compteur pour parcourir les dés
        int indexGarde = 0;         // compteur pour saisir les indices des dés à garder
        ...
        while (indexDe < 5)
        {
            des[indexDe] = alea.Next(1, 7);
            desGardes[indexDe] = false;
            indexDe = indexDe + 1;
        }
    ...
    public static void SaisieCategorie(int[] scoresJoueur, int[] des, string[] nomCategorie, out int categorieChoisie)
    {
        ...
        while (indexCategorie < 13)
        {
            if (scoresJoueur[indexCategorie] == -1)
            {
                if (indexCategorie < 6)
                {
                    CalculerScoreChiffre(des, indexCategorie + 1, out scoreTemp);
                }
                else if (indexCategorie == 6)
                {
                    CalculerBrelanCarre(des, 3, out scoreTemp);
                }
                else if (indexCategorie == 7)
                {
                    CalculerBrelanCarre(des, 4, out scoreTemp);
                }
                else if (indexCategorie == 8)
                {
                    CalculerFullHouse(des, out scoreTemp);
                }
                else if (indexCategorie == 9)
                {
                    CalculerSuite(des, 4, out scoreTemp);
                }
                else if (indexCategorie == 10)
                {
                    CalculerSuite(des, 5, out scoreTemp);
                }
                else if (indexCategorie == 11)
                {
                    CalculerChance(des, out scoreTemp);
                }
                else if (indexCategorie == 12)
                {
                    CalculerYahtzee(des, false, out scoreTemp, out yahtzeeTemp);
                }
                Console.WriteLine((indexCategorie + 1) + ". " + nomCategorie[indexCategorie] + " : " + scoreTemp + " points");
            }
            indexCategorie = indexCategorie + 1;
        }
