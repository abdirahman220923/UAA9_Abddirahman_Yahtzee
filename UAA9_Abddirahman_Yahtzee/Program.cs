...
class Program
{
    static void Main(string[] args)
    {
        ...
        int indexJoueur;                // compteur général
        ...
        do
        {
            FonctionProgram.LireEntier("Combien de joueurs ? (1 à 4) :", out nbJoueurs);

            while (nbJoueurs < 1 || nbJoueurs > 4)
            {
                Console.WriteLine("Nombre invalide ! (1-4)");
                FonctionProgram.LireEntier("Combien de joueurs ? (1 à 4) :", out nbJoueurs);
            }

            FonctionProgram.GestionJoueurs(nbJoueurs, out nomsJoueurs, out scoresJoueurs);
            ...
            while (indexJoueur < nbJoueurs)
            {
                ...
                {
                    scoresChiffres[indexChiffre] = scoresCategories[indexJoueur][indexChiffre];
                    indexChiffre = indexChiffre + 1;
                }
                FonctionProgram.CalculerBonus(scoresChiffres, out bonus);
                scoresTotaux[indexJoueur] = scoresTotaux[indexJoueur] + bonus;
                if (bonus > 0)
                {
                    Console.WriteLine(nomsJoueurs[indexJoueur] + " obtient le bonus de 35 points !");
                }
            ...
            while (indexJoueur < nbJoueurs)
            {
                FonctionProgram.AfficherFeuilleScore(nomsJoueurs[indexJoueur], scoresCategories[indexJoueur], scoresTotaux[indexJoueur]);
                indexJoueur = indexJoueur + 1;
            }
